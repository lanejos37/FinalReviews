...
(define-struct (keydoor door)
  ;;key is an object to open the door
  (key
   ;;locked is a boolean whether or not the door is locked
   locked))
(define-struct (codedoor door)
  ;;code is a string
  (code
   locked))
...
(define (unlock keydoor key)
  (if (have? key)
      (set-keydoor-locked! keydoor #false)
      (display-line "You need the key!")))

(define (open keydoor)
  (if (equal? (keydoor-locked keydoor) #false)
      (go keydoor)
      (display-line "You need to unlock the door!")))

(define (unbolt codedoor string)
  (if (string=? (codedoor-code codedoor) string)
      (set-codedoor-locked! codedoor #false)
      (display-line "You need the code!")))

(define (gothru codedoor)
  (if (equal? (codedoor-locked codedoor) #false)
      (go codedoor)
      (display-line "You need to unlock the door!")))

(define (unseal safe string)
  (if (string=? (safe-code safe) string)
      (take (safe-content safe))
      (display-line "You need the code!")))

(define (peruse sign)
  (display-line (sign-writing sign)))
...
(define (start-game)
  ...
  (local [(define room1 (new-room "First"))
          (define room2 (new-room "Second"))
          (define room3 (new-room "Final"))]
    (begin (set! me (new-person "" room1))
           (local [(define x (make-codedoor (list "super" "fun")
                                            '()
                                            room1 room2
                                            "CS111"
                                            #true
                                            ))]
             (begin (initialize-thing! x) (void)))
           (local [(define y (make-keydoor (list "locked")
                                           '()
                                           room2 room3
                                           "K"
                                           #true
                                           ))]
             (begin (initialize-thing! y) (void)))
           (local [(define z (make-sign (list "helpful")
                                        '()
                                        room1
                                        "CS111"))]
             (begin (initialize-thing! z) (void)))
           (local [(define x (make-sign (list "another helpful")
                                        '()
                                        room2
                                        "Ian"))]
             (begin (initialize-thing! x) (void)))

           (local [(define x (make-safe (list "important")
                                        '()
                                        room2
                                        "Ian"
                                        (make-key (list "door")
                                                  ...
                                                  "K")))]
             (begin (initialize-thing! x) (void)))
           (local [(define z (make-sign (list "congratulations")
                                        '()
                                        room3
                                        "Yay! You did it!"))]
             (begin (initialize-thing! z) (void)))
